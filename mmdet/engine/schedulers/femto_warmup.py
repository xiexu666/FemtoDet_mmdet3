...
class FemtoWarmupParamScheduler(_ParamScheduler):
    r"""Warm up the parameter value of each parameter group by Femto
    formula:
    ...
    def _get_value(self):
        ...
        if self.last_step == 0:
            return [
                base_value * self.warmup_ratio * pow(
                (self.last_step + 1) / float(self.total_iters), 2)
                for base_value in self.base_values
            ]

        return [
            base_value *
            self.warmup_ratio * pow(
                (self.last_step + 1) / float(self.total_iters), 2)
...
@PARAM_SCHEDULERS.register_module()
class FemtoWarmupLR(LRSchedulerMixin, FemtoWarmupParamScheduler):
    """Warm up the learning rate of each parameter group by Femto formula.
...
@PARAM_SCHEDULERS.register_module()
class FemtoWarmupMomentum(MomentumSchedulerMixin,
                              FemtoWarmupParamScheduler):
    """Warm up the momentum value of each parameter group by Femto formula.
